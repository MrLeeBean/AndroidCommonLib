...
public class XEditText extends AppCompatEditText {
    ...
    private void initAttrs(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (!disableClear) {
            ...
            if (cdId == -1)
                cdId = R.drawable.base_x_et_svg_ic_clear;
            mClearDrawable = ContextCompat.getDrawable(context, cdId);
            mClearDrawable.setBounds(0, 0, mClearDrawable.getIntrinsicWidth(),
                    mClearDrawable.getIntrinsicHeight());
            if (cdId == R.drawable.base_x_et_svg_ic_clear)
        ...
        if (!disableTogglePwdDrawable && (inputType == 129 || inputType == 145 || inputType == 18 || inputType == 225)) {
            ...
            if (mShowPwdResId == -1)
                mShowPwdResId = R.drawable.base_x_et_svg_ic_show_password;
            if (mHidePwdResId == -1)
                mHidePwdResId = R.drawable.base_x_et_svg_ic_hide_password;

            int tId = isPwdShow ? mShowPwdResId : mHidePwdResId;
            mTogglePwdDrawable = ContextCompat.getDrawable(context, tId);
            if (mShowPwdResId == R.drawable.base_x_et_svg_ic_show_password ||
                    mHidePwdResId == R.drawable.base_x_et_svg_ic_hide_password) {
            ...
            if (!disableClear) {
                mBitmap = getBitmapFromVectorDrawable(context, cdId,
                        cdId == R.drawable.base_x_et_svg_ic_clear); // clearDrawable
            }
        }
